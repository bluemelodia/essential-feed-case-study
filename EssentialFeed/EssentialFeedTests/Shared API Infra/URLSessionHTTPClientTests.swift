...
class URLSessionHTTPClientTests: XCTestCase {
	...
	func test_cancelGetFromURLTask_cancelsURLRequest() {
		...
		URLProtocolStub.observeRequests { _ in
			print("===> [test] fulfill exp")
			exp.fulfill()
		}

		let receivedError = resultErrorFor(taskHandler: {
			print("===> [test] canceling task")
			$0.cancel()
		}) as NSError?
		wait(for: [exp], timeout: 1.0)

		print("===> [test] asserting received error code")
	...
	private func resultErrorFor(_ values: (data: Data?, response: URLResponse?, error: Error?)? = nil, taskHandler: (HTTPClientTask) -> Void = { _ in }, file: StaticString = #filePath, line: UInt = #line) -> Error? {
		print("==> [resultErrorFor] start")
		let result = resultFor(values, taskHandler: taskHandler, file: file, line: line)
		print("===> [resultErrorFor] result: \(result)")

		switch result {
		case let .failure(error):
			print("===> [resultErrorFor] failure: \(error)")
			return error
		default:
			print("===> [resultErrorFor] default")
	...
	private func resultFor(_ values: (data: Data?, response: URLResponse?, error: Error?)?, taskHandler: (HTTPClientTask) -> Void = { _ in },  file: StaticString = #filePath, line: UInt = #line) -> HTTPClient.Result {
		...
		taskHandler(sut.get(from: anyURL()) { result in
			print("==> [resultFor] about to set received result: \(result)")
			receivedResult = result
			exp.fulfill()
		})
		
		wait(for: [exp], timeout: 1.0)
		print("==> [resultFor] return received result: \(receivedResult)")
